...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \textit{For simulation}, this memory will ignore bits from (31 to 8) inclusive, and only work with bits from (7 to 0) inclusive.
        \item \textbf{Data Bus Width}: 16 bits.
        \item \textbf{Total Number of Words}: $2^{32}$ words = 4,294,967,296 words = 8,589,934,592 bytes.\\
        \textit{For simulation}, this memory will only have: $2^{8}$ words = 256 words = 512 bytes.
