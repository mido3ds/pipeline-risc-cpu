...
\subsection{Inputs}
\begin{itemize}
    \item PC Incremented Address (32 bits)
    \item Register Address (32 bits)
    \item OpCode (7 bits)
    \item Hash Update (2 bits)
\end{itemize}

\subsection{Outputs}
\begin{itemize}
    \item PC predicted (32 bits)
\end{itemize}

\subsection{Logic}
\begin{itemize}
    \item Check if OpCode is of a "JZ" instruction, if true:
    \begin{itemize}
        \item Check the Least Significant bit of the 2-bits-hash table
        \item If 0 (Leave it):
        \begin{itemize}
            \item PC predicted = PC incremented
        \end{itemize}
        \item If 1 (Taken):
        \begin{itemize}
            \item PC predicted = Register Address
        \end{itemize}
    \end{itemize}
    \item Check if Update Hash first bit is '1', if true:
        \begin{itemize}
            \item update the hash with the second bit.
        \end{itemize}
\end{itemize}
...
\begin{itemize}
    \item PC Predicted (32 bits)
    \item PC from Register (32 bits)
    \item PC incremented
    \item OpCode (7 bits)
    \item Zero Flag (1 bit)
\end{itemize}

\subsection{Outputs}
\begin{itemize}
    \item IF Flush (1 bit)
    \item Update hash table (2 bits)
    \item Correct Address (32 bits)
\end{itemize}
...
\begin{itemize}
    ...
    \item If true:
    \begin{itemize}
        \item (Predict == Reg.) equals zero if the predicted PC does not equal the Reg. address value.
       \begin{center}
        \captionof{table}{Branch Address Unit\label{tab:1op}}
        \begin{tabular}{||c| c| c| c| p{40mm}||} 
             \hline
             Zero Flag & Predict == Reg. & Flush & Correct Adds  \\ [0.5ex] 
             \hline\hline
             0 & 0 & 0 & as is & ----------------------- \\
             \hline
             0 & 1 & 1 & PC++ & ----------------------- \\
...
% \section{SPU at Decode}

% \subsection{Inputs}
% \begin{itemize}
%     \item Stack Pointer (32 bits)
%     \item Stack Pointer star (32 bits), the incremented SP (assumption)
%     \item PC incremented (32 bits)
%     \item OpCode (7 bits)
% \end{itemize}

% \subsection{Outputs}
% \begin{itemize}
%     \item Address to the memory (SP) (32 bits)
%     \item Data to the memory (to be pushed) (32 bits)
%     \item The updated value of SP* (32 bits)
%     \item Selector (1 bit)
%     \item Read/Write Control (2 bits)
% \end{itemize}

% \subsection{Idea}
% \begin{itemize}
%     \item During the 1st half of the fetch stage, we make an assumption that the upcoming operation is POP or RET, thus incremenitng (2) the stack pointer and store it at SP*.
%     \item During the 2nd half of the fetch, if the operation is not even using the Stack, no operation is executed, thus SP remains the same.
%     \item If it was a stack operation, insert/remove, at the Decode stage we have two stack pointers SP and SP*, SP is used for insertion, SP* is used for removing, so, at the 1st half of the decode stage, we access the memory, asking for our operation, whether it was push/call/ret/pop.
%     \item 
%     \begin{itemize}
%         \item For Push, address will be SP, and data value will be Register value.
%         \item For Call, address will be SP, and data value will be PC++
%         \item For Pop, address will be SP*, and the mem_out data is Register value.
%         \item For Ret, address will be SP*, and the mem_out data is PC
%     \end{itemize}
%     \item Also during the 1st half, if it was insertion (push or call), the SP* gets updated with (SP--), because we are not using it.
%     \item During the second half of the cycle, 
% \end{itemize}

% \subsection{Logic}
% \begin{itemize}
%     \item Assuming its one of these four operations (Push, Pop, Call, Ret), two of them (Push, Call) insert/stores data at the Stack, and the other two (Pop, Ret) reads from the Stack.
%     \item At the fetch stage we incremented the SP and store it at SP*.
%     \item At the Decode stage's first half, if the operation is inserting (Push, Call):
%     \begin{itemize}
%         \item The address to the memory will be SP, regularly
%         \item and the SP* gets updated with decremented value of SP, and this will not cause an issue because the has-been incremented value stored at SP* is not used.
%     \end{itemize}
%     \itemIf the operation is Reading (Pop, Ret):
%     \begin{itemize}
%         \item The address to the memory will be SP*
%         \item and the SP* can not be updated
%     \end{itemize}
%     \item At the Decode stage's second half, the SP is updated with SP*.
% \end{itemize}
...
\begin{itemize}
    \item \textbf{OPCode Checker:} checks the opcode of the current instruction to check whether it will cause data hazard or not, then activates the Register Comparator accordingly.
...
We use 2-bit branch predictor, which is a \emph{Finite State Machine} (FSM) to predict whether the branch will be taken (1) or not (0).
